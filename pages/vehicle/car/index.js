...
const Car = () => {
  ...
  // const pageInfo = dataArr[dataArr.length - 1]
  useEffect(() => {
    if(data?.vehicleFilter?.edges){
      const currency = binarySearch("Norway")
      cleanFilterData(data, "vehicleFilter", currency, setDataArr)
      setPageInfo(data?.vehicleFilter?.pageInfo)
    }
  }, [data]);
                        ...
                        <Link href="">
                          <a>
                            <Image
                              loader={myLoader}
                              src={`${Cars[0]["image"]}`}
                              alt={item?.categoryName ? item?.categoryName : Cars[0]["categoryName"]}
                              width="498"
                              height="268"
...
======= */}
                          {item?.title ? item["title"] : Cars[0]["title"]}
                          <div className="text-sm font-normal">{`City: ${item?.city ? item["city"] : Cars[0]["title"]}, 
                            ${item?.street_address ? item["street_address"] : Cars[0]["street_address"]}`
                          }</div>
